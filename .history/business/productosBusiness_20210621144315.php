...
require_once(DIR_BASE.'DAO/productosDao.php');
require_once(DIR_BASE.'helpers/image.php');
...
function saveImage($datos,$id){ 
    ...
    if(is_array($datos['name'])){
        ...
        foreach($datos['name'] as $index => $name){ 
            redimensionar($ruta,$datos['name'][$index],$datos['tmp_name'][$index],$index,$tamanhos);
        }
    }else{
