...
function businessGuardarProducto($datos = array()){
    ...
    if(!empty($_FILES['imagen'])){
        if(!is_dir(DIR_BASE.'images/carne/gourmet'.$id)){
            mkdir(DIR_BASE.'images/carne/gourmet'.$id);
        }
        move_uploaded_file($_FILES['imagen']['tmp_name'],DIR_BASE.'images/carne/gourmet'.$id.'/'.$_FILES['imagen']['name']);
        if(file_exists(DIR_BASE.'images/carne/gourmet'.$id.'/'.$datos['old_imagen'])){
            unlink(DIR_BASE.'images/carne/gourmet'.$id.'/'.$datos['old_imagen']);
        } 
    } 
